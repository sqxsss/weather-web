  ...
  <div style="width: 1450px; margin: 0 auto">
    ...
    <div>
      <el-table :data="tableData" height="700px" border style="width: 100%">
        ...
        <el-table-column prop="mxpn" label="MaxTemperatureInEvaPan" width="250px"></el-table-column>
        <el-table-column prop="mnpn" label="MinTemperatureInEvaPan" width="250px"></el-table-column>
        <el-table-column prop="evap" label="EvaporationOfWater" width="250px"></el-table-column>
      </el-table>
      <el-pagination
  ...
  methods: {
    ...
    getTemperatureList () {
      ...
      if (this.dateValue === null) {
        ...
        this.$ajax.post('/api/basicData/evaporation/getListByDate/' + this.dateValue[0] + '/' + this.dateValue[1]).then(res => {
          if (res.status === 200) {
            this.currentPage = 1
        ...
        this.$ajax.post('/api/basicData/evaporation/getListByCityDate/' + this.city + '/' + this.dateValue[0] + '/' + this.dateValue[1]).then(res => {
          if (res.status === 200) {
            this.currentPage = 1
            this.totalData = res.data
            this.total = this.totalData.length
        ...
        this.$ajax.post('/api/basicData/evaporation/getListByStationDate/' + this.station + '/' + this.dateValue[0] + '/' + this.dateValue[1]).then(res => {
          if (res.status === 200) {
            this.currentPage = 1
            this.totalData = res.data
            this.total = this.totalData.length
