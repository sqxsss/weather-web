<template>
  <div>
    ...
    <div id="barchartMT" style="width: 95%;height: 750px; margin: 0 auto"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getChartData () {
      this.$ajax.post('/api/complexQuery/temperatureComp/getMonthlyDataListByCity/' + this.yearValue + '/' + this.cityValue.replaceAll('%20', ' ')).then(res => {
        if (res.status === 200) {
          this.monthlyAvgTem = res.data
          this.$nextTick(() => {
            this.setTest()
          })
        }
    ...
    setTest () {
      ...
      myChart = echarts.init(document.getElementById('barchartMT'))
      let option = {
        title: {
          text: 'Monthly Temperature Statistics'
        },
        tooltip: {
          trigger: 'axis'
        },
