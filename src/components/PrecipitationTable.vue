  ...
  <div style="width: 1450px; margin: 0 auto">
    ...
    <div>
      <el-table :data="tableData" height="700px" border style="width: 100%">
        ...
        <el-table-column prop="dapr" label="Multi-Precipitation" width="200px"></el-table-column>
        <el-table-column prop="mdpr" label="NumberOfDay" width="180px"></el-table-column>
        <el-table-column prop="snow" label="Snowfall" width="180px"></el-table-column>
        <el-table-column prop="snwd" label="Snow depth" width="180px"></el-table-column>
        <el-table-column prop="prcp" label="Precipitation" width="180px"></el-table-column>
      </el-table>
      <el-pagination
  ...
  methods: {
    ...
    getTemperatureList () {
      ...
      if (this.dateValue === null) {
        ...
        this.$ajax.post('/api/basicData/precipitation/getDataByTime/' + this.dateValue[0] + '/' + this.dateValue[1]).then(res => {
          if (res.status === 200) {
            this.currentPage = 1
        ...
        let s = 'null'
        this.$ajax.post('/api/basicData/precipitation/getDataByCityStationTime/' + s + '/' + this.dateValue[0] + '/' + this.dateValue[1] + '/' + this.city).then(res => {
          if (res.status === 200) {
            this.currentPage = 1
            this.totalData = res.data
            this.total = this.totalData.length
        ...
        this.$ajax.post('/api/basicData/precipitation/getDataByStationTime/' + this.station + '/' + this.dateValue[0] + '/' + this.dateValue[1]).then(res => {
          if (res.status === 200) {
            this.currentPage = 1
            this.totalData = res.data
            this.total = this.totalData.length
